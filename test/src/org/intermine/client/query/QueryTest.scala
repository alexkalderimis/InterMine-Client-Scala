  ...
  def typedresults(): Unit = {
    ( (s from "Employee")                                      >>=
      ...
      .fold(Assert.fail, rows => {
        rows groupBy (_(2)) map
          {case (d, rs) => (d, (rs map (_(1)) filter (!_.isEmpty) map {case Some(v) => v.toInt}).sum, rs.size)} map {
            case (None, i, t) => println("No department: " + i + " avg(" + i / t + ")")
            case (Some(d), i, t) => println(d + ": " + i + " avg(" + i / t + ")")
        }
        println("Total: " +  (rows map (_(1)) map {case Some(v) => v.toInt}).sum)
      })
  }

  @Test
  def nullResults(): Unit = {
    ( (s from "Employee") >>= (_.select("age", "end")) >>= (_.rows) )
      .fold(Assert.fail, rows => {
        val display: String => Int => Int => Unit =
          end => sum => total => println(end + ": " + sum + " avg(" + sum/total + ")")
        rows groupBy (_.get("end")) map {case (d, rs) => (d, (rs map (_("age").toInt)).sum, rs.size)} map {
          case (None, i, t) => display("No end")(i)(t)
          case (Some(end), i, t) => display(end)(i)(t)
        }
        println("Avg: " +  (rows map (_("age").toInt)).sum / rows.size)
      })
  }


  @Test
  def assocResults(): Unit = {
    ( (s from "Employee") >>= (_.select("age", "end")) >>= (_.all) )
      .fold(Assert.fail, rows => {
        val display: String => Int => Int => Unit =
          end => sum => total => println(end + ": " + sum + " avg(" + sum/total + ")")
        rows groupBy (_(1)) map {case (d, rs) => (d, (rs map (_(0)) map {case Some(v) => v.toInt; case None => 0}).sum, rs.size)} map {
          case (None, i, t) => display("No end")(i)(t)
          case (Some(end), i, t) => display(end)(i)(t)
        }
        println("Avg: " +  (rows map (_(0)) map {case Some(v) => v.toInt; case None => 0}).sum / rows.size)
      })
  }
